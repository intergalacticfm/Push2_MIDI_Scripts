...
# [GCC 6.3.0 20170118]
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\real_time_channel.py
...
class RealTimeDataComponent(Component):
    ...
    def attach(self):
        if self._valid or self.is_enabled():
            data = self._data if 1 else None
            if data != None:
