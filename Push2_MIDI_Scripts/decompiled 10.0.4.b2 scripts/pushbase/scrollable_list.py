...
# [GCC 6.3.0 20170118]
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\scrollable_list.py
    ...
    def select_item_index_with_border(self, index, border_size):
        ...
        if self.fixed_offset is not None:
            ...
            if index >= 0 and index < len(self._items):
                if in_range(index, self._offset + border_size, self._offset + self._num_visible_items - border_size) or self.selected_item_index < index:
                    offset = index - (self._num_visible_items - 2 * border_size) if 1 else index - border_size
