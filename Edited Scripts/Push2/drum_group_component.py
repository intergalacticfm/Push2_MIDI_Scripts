...
# [GCC 6.3.0 20170118]
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\drum_group_component.py
    ...
    def _chain_color_for_pad(self, pad, color):
        if color == 'DrumGroup.PadFilled':
            ...
            if color == 'DrumGroup.PadMuted':
                assert pad.chains and liveobj_valid(pad.chains[0]), 'Filled pads should have a chain'
                color = IndexedColor.from_live_index(pad.chains[0].color_index, shade_level=2)
        return color
