...
class PushBase(ControlSurface):
    ...
    def _create_alternating_layout_modes_for_64pads(self, mode='drum', add_touch_strip=False, default_mode=None, alternative_mode=None):
        ...
        loop_selector = LoopSelectorComponent(follow_detail_clip=True, clip_creator=self._clip_creator, name=mode.title() + '_Pad_Loop_Selector', is_enabled=False, layer=Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
         0]), default_size=8)
        alternating_layouts = MessengerModesComponent(muted=True, is_enabled=False)
    ...
    def _switch_note_mode_layout(self):
        cyclable_mode = {'instrument': self._instrument,
           ...
           'slicing': self._slicing_modes
           }.get(self._note_modes.selected_mode, None)
    ...
    def _init_transport_and_recording(self):
        ...
        self._transport.layer = Layer(play_button='play_button', stop_button=self._with_shift('play_button'), metronome_button='metronome_button')
    ...
    def _create_sequence_instrument_layer_with_loop(self):
        return Layer(playhead='playhead_element', mute_button='global_mute_button', quantization_buttons='side_buttons', loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
         0], note_editor_matrices=ButtonMatrixElement([[ self.elements.matrix.submatrix[:, 7 - row] for row in xrange(7) ]]), duplicate_button='duplicate_button', delete_button='delete_button')

    def _create_play_instrument_with_loop_layer(self):
        return Layer(playhead='playhead_element', mute_button='global_mute_button', quantization_buttons='side_buttons', loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:,
         0], duplicate_button='duplicate_button', delete_button='delete_button')
    ...
    def _init_split_melodic_sequencer(self):
        ...
        self._split_sequencer_mode.add_mode('alternative', [
         ...
         AddLayerMode(self._split_melodic_sequencer, Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[:,
          ...
          4]))], message=consts.MessageBoxText.LAYOUT_MELODIC_32_PADS_LOOP_SELECTOR, default_mode='default', alternative_mode='alternative')
    ...
    def _init_value_components(self):
        self._swing_amount = ValueComponent('swing_amount', self.song, display_label='Swing Amount:', display_format='%d%%', model_transform=lambda x: clamp(x / 200.0, 0.0, 0.5), view_transform=lambda x: x * 200.0, encoder_factor=100.0, encoder_touch_delay=TEMPO_SWING_TOUCH_DELAY)
        ...
        self._master_vol.layer = Layer(encoder=self._with_shift('master_volume_control'))
        ...
        self._master_cue_vol.layer = Layer(encoder='master_volume_control')
