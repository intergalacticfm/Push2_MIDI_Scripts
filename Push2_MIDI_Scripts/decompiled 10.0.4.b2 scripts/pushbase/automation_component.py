...
# [GCC 6.3.0 20170118]
# Embedded file name: c:\Jenkins\live\output\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\automation_component.py
# Compiled at: 2018-09-07 02:55:17
...
class AutomationComponent(DeviceParameterComponent):
    ...
    def _update_parameter_floats(self):
        if self._clip and self.is_enabled():
            parameters = self.parameters
            envelopes = [ self._clip.automation_envelope(self._parameter_for_index(parameters, index)) if 1 else None for index, param in enumerate(parameters) if param != None
                        ]
            self._parameter_floats = [ [ self._value_at_time(envelope, step) if 1 else 0.0 for envelope in envelopes if envelope != None ] for step in self.selected_time
                                     ]
        else:
            self._parameter_floats = []
        return
